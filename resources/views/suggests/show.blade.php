...
@section('container')
    ...
    <div class="w-10/12 border-t-4 border-blue-300 mx-auto">
        <div class="pt-4 pb-16" >
            @foreach($cmts as $cmt)
                <div class="mt-4 shadow-md" style="min-height: 80px;">
                    <div id="{{$cmt->id}}cmtBox">
                        ...
                        @auth()
                            <div class="py-1 text-right text-base">
                                @if($cmt->user_id == auth()->user()->id || auth()->user()->is_admin == 1)
                                    ...
                                    <div class="pr-2 inline-block">
                                        ...
                                        <form action="{{route($sug->board_name.'.comments.destroy', [$sug->id, $cmt->id])}}" method="post" class="inline-block">
                                            @csrf
                    ...
                    <div id="{{$cmt->id}}editBox" class="py-8 hidden">
                        <form action="{{route($sug->board_name.'.comments.update', [$sug->id, $cmt->id])}}" method="post">
                            @csrf
                            @method('put')
        ...
        @auth()
            <div class="border-t-4 border-b-4 border-gray-300 py-8">
                <form action="{{route($sug->board_name.'.comments.store', $sug->id)}}" method="post">
                    @csrf
                    <label for="story" class="hidden"></label>
