    ...
    @auth()
        @if($post->user_id == auth()->user()->id)
            <div class="w-10/12 mx-auto my-4 text-lg">
                <span class="hover:text-blue-300"><a href="{{route($post->mbti_board.'.edit', $post->id)}}"><i class="xi-pen-o pr-2"></i><button>수정</button></a></span>
                <form action="{{route($post->mbti_board.'.destroy', $post->id)}}" method="post" class="inline-block">
                    @csrf
                    @method('delete')
