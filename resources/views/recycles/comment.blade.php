        ...
        @foreach($cmts as $cmt)
                <div class="mt-4 shadow-md @if($cmt->class != 0) w-10/12 ml-auto @endif" style="min-height: 80px;">
                        ...
                        <div class="py-1 text-right text-base">
                            ...
                            @if($cmt->user_id == auth()->user()->id)
                                <div class="pr-2 inline-block">
                                    ...
                                    <form action="{{route($post->mbti_board.'.comments.destroy', [$post->id, $cmt->id])}}" method="post" class="inline-block">
                                        @csrf
                    ...
                    <div id="{{$cmt->id}}editBox" class="py-8 hidden">
                        <form action="{{route($post->mbti_board.'.comments.update', [$post->id, $cmt->id])}}" method="post">
                            @csrf
                            @method('put')
...
{{--  댓글 작성  --}}
    @auth()
        ...
        <form action="{{route($post->mbti_board.'.comments.store', $post->id)}}" method="post">
            @csrf
            <label for="story" class="hidden"></label>
